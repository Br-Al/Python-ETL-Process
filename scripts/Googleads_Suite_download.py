...
from utils.get_google_ads_report import get_campaign_data, get_ads_data, get_geo_performance_data, get_landing_Page_data,get_keyword_view_data,get_bidding_strategy_data,get_ads_with_bid_data,get_campaign_bidding_data,get_ads_bid_craft_AI_data,get_ads_bid_craft_ai_conv_data,get_campaign_budget_data
...
if __name__ == '__main__':
    try:
        ...
        account_id = cred_json["pdf_suite_account_id"]

        api_client = google_ads_authentication(cred_json)
        max_campaign_date_query = """SELECT MAX(Date)as new_date
                FROM [GoogleAds].[CampaignPerformance_V2] WHERE [CustomerId]='8827526341'"""
        ...
        max_ad_date_query = """SELECT MAX(Date)as new_date
                                       FROM [GoogleAds].[AdgroupPerformance_V2] WHERE [CustomerId]='8827526341'"""
        ...
        max_landing_date_query = """SELECT MAX(Date)as new_date
                        FROM [GoogleAds].[LandingPage_V2] WHERE [CustomerId]='8827526341'"""
        ...
        max_geo_date_query = """SELECT MAX(Date)as new_date
                                       FROM [GoogleAds].[GeoPerformance_V2] WHERE [CustomerId]='8827526341'"""
        ...
        max_campaign_bid_date_query = """SELECT MAX(Date)as new_date
                                         FROM [GoogleAds].[CampaignPerformance_V2Bid] WHERE [CustomerId]='8827526341'"""
        ...
        max_keyword_date_query = """SELECT MAX(Date)as new_date
                                            FROM [GoogleAds].[KeywordPerformanceReport_V2] WHERE [CustomerId]='8827526341'"""
        ...
        max_ads_craft_ai_query = """SELECT MAX(Date)as new_date
                                                      FROM [GoogleAds].[AdgroupPerformance_V2CraftAI] WHERE [CustomerId]='8827526341'"""

        max_ads_processed_craft_ai_date = db.get_query_df(max_ads_craft_ai_query)
        max_ads_craft_ai_date = max_ads_processed_craft_ai_date.iat[0, 0]
        ads_craft_ai_start_date = get_max_date(max_ads_craft_ai_date)
        ads_bid_craft_ai_df = get_ads_bid_craft_AI_data(api_client, account_id, ads_craft_ai_start_date, e_date)
        ads_bid_craft_ai_df['BiddingStrategyType'] = ads_bid_craft_ai_df['BiddingStrategyType'].map(map_bidding_data)
        ads_bid_craft_ai_df['CampaignBiddingStrategyType'] = ads_bid_craft_ai_df['CampaignBiddingStrategyType'].map(
            map_bidding_data)
        ads_bid_craft_ai_df['CreateDate'] = datetime.now().strftime("%Y-%m-%d")

        if not ads_bid_craft_ai_df.empty:
            insert_query = """INSERT INTO [GoogleAds].[AdgroupPerformance_V2CraftAI] (
                                  [Date]
                                 ,[CustomerID]
                                 ,[CustomerDescriptiveName]
                                 ,[CurrencyCode]
                                 ...
                                 ,[AdGroupID]
                                 ,[AdGroupName]
                                 ,[AdGroup_effective_target_roas]
                                 ,[AdGroup_effective_target_cpa_micros]
                                 ,[AdGroup_cpc_bid_micros]
                                 ,[AdGroup_cpv_bid_micros]
                                 ,[AdGroup_target_cpa_micros]
                                 ,[AdGroup_target_roas]
                                 ,[BiddingStrategyId]
                                 ,[BiddingStrategyName]
                                 ,[BiddingStrategyType]
            ...
            db.insert_many(ads_bid_craft_ai_df, insert_query)

        max_ads_craft_ai_conv_query = """SELECT MAX(Date)as new_date
                                            FROM [GoogleAds].[AdgroupPerformance_V2CraftAIConv] WHERE [CustomerId]='8827526341'"""

        max_ads_processed_craft_ai_conv_date = db.get_query_df(max_ads_craft_ai_conv_query)
        max_ads_craft_ai_con_date = max_ads_processed_craft_ai_conv_date.iat[0, 0]
        ads_craft_ai_conv_start_date = get_max_date(max_ads_craft_ai_con_date)
        ads_craft_ai_conv_df = get_ads_bid_craft_ai_conv_data(api_client, account_id, ads_craft_ai_conv_start_date,e_date)
        ads_craft_ai_conv_df['CreateDate'] = datetime.now().strftime("%Y-%m-%d")

        if not ads_craft_ai_conv_df.empty:
            insert_query = """INSERT INTO [GoogleAds].[AdgroupPerformance_V2CraftAIConv] (
                                             [Date]
                                              ,[CustomerID]
                                              ,[CampaignId]
                                              ,[CampaignName]
            ...
            db.insert_many(ads_craft_ai_conv_df, insert_query)

        max_campaign_budget_query = """SELECT MAX(Date)as new_date
                                                    FROM [GoogleAds].[CampaignPerformance_V2Budget] WHERE [CustomerId]='8827526341'"""

        max_campaign_budget_processed_date = db.get_query_df(max_campaign_budget_query)
        max_campaign_budget_date = max_campaign_budget_processed_date.iat[0, 0]
        campaign_budget_start_date = get_max_date(max_campaign_budget_date)
        campaign_budget_df = get_campaign_budget_data(api_client, account_id, campaign_budget_start_date, e_date)
        campaign_budget_df['CreateDate'] = datetime.now().strftime("%Y-%m-%d")

        if not campaign_budget_df.empty:
            insert_query = """INSERT INTO [GoogleAds].[CampaignPerformance_V2Budget] (
                                         [Date]
                                         ,[CustomerId]
                                         ,[CampaignId]
                                         ,[CampaignName]
                                         ...
                                         ,[Cost]
                                         ,[CreateDate] ) VALUES (?, ?, ?, ?, ?, ?,?,?,?,?,?,?)"""
            db.insert_many(campaign_budget_df, insert_query)

        max_bidding_date_query = """SELECT MAX(Date)as new_date
                                            FROM [GoogleAds].[BiddingStrategyReport_V2] WHERE [CustomerId]='8827526341'"""
        ...
        max_ad_bid_date_query = """SELECT MAX(Date)as new_date
                                       FROM [GoogleAds].[AdgroupPerformance_V2Bid] WHERE [CustomerId]='8827526341'"""
